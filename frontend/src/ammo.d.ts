/*
 * A copy of https://github.com/giniedp/ammojs-typed with a few tweaks.
 * The author of the original .d.ts file is https://github.com/giniedp
 */

declare function Ammo<T>(target?: T): Promise<T & typeof Ammo>;
declare namespace Ammo {
    function castObject<T>(obj: unknown, toType: class<T>): T;
    ...
    class btIDebugDraw {
        /**
         * @param from The byte index of the first line point.
         * The actual coordinates are from {@code Ammo.HEAPF32[from / 4]} to {@code Ammo.HEAPF32[from / 4 + 2]}.
         *
         * @param to The byte index of the second line point.
         * The actual coordinates are from {@code Ammo.HEAPF32[to / 4]} to {@code Ammo.HEAPF32[to / 4 + 2]}.
         *
         * @param color The byte index of the color.
         * The actual color is from {@code Ammo.HEAPF32[color / 4]} to {@code Ammo.HEAPF32[color / 4 + 2]}.
         */
        drawLine(from: number, to: number, color: number): void;
        /**
         * @param pointOnB The byte index of the contact point.
         * The actual coordinates are from {@code Ammo.HEAPF32[pointOnB / 4]} to {@code Ammo.HEAPF32[pointOnB / 4 + 2]}.
         *
         * @param normalOnB The byte index of the contact normal.
         * The actual coordinates are from {@code Ammo.HEAPF32[normalOnB / 4]} to
         * {@code Ammo.HEAPF32[normalOnB / 4 + 2]}.
         *
         * @param color The byte index of the color.
         * The actual color is from {@code Ammo.HEAPF32[color / 4]} to {@code Ammo.HEAPF32[color / 4 + 2]}.
         */
        ...
        /**
         * @param location The byte index of the text location.
         * The actual coordinates are from {@code Ammo.HEAPF32[location / 4]} to {@code Ammo.HEAPF32[location / 4 + 2]}.
         *
         * @param textString The text to draw.
         */
        draw3dText(location: number, textString: string): void;
    ...
    class btManifoldPoint {
        ...
        get_m_userPersistentData(): unknown;
        set_m_userPersistentData(m_userPersistentData: unknown): void;
    }
    ...
    class btBroadphaseInterface {
         ...
         * See https://github.com/kripken/ammo.js/issues/254#issuecomment-496729110
         * It does not explain why it's missing, but it does show that I'm not crazy for seeing that it's absent.
         *
         * You can get the overlapping pair cache in another way - through {@link btCollisionWorld#getPairCache}
         */
        getOverlappingPairCache(): never;
    }
    ...
    class RaycastInfo {
        ...
        get_m_groundObject(): unknown;
        set_m_groundObject(m_groundObject: unknown): void;
    }
    ...
    // Trying to instantiate an ActionInterface results in a "cannot construct a btActionInterface, no constructor in
    //  IDL" error
    // Also, the class is abstract in C and Java.
